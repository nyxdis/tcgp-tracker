...
from tracker.models import PokemonSet, Card, Rarity, RarityProbability, Version

class Command(BaseCommand):
    help = 'Imports Pokémon sets, cards, rarities and rarity probabilities from CSV files'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--rarities', type=str, help='Path to CSV file with rarity data')
        parser.add_argument('--rarityprob', type=str, help='Path to CSV file with rarity probability data')
                ...
                if pack_names:
                    for pack_name in pack_names.split('|'):
                        ...
                        if not pack_obj:
                            ...
                            if not version:
                                self.stderr.write(f"No Version found, cannot create pack '{pack_name}' for card '{card_obj.name}'")
                                continue
                            # Pack neu anlegen
                            pack_obj = pset.packs.create(name=pack_name, rarity_version=version)
                            self.stdout.write(f"→ Created new pack '{pack_name}' with version '{version.name}'")
        ...
        with open(filepath, newline='', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                try:
                    ...
                    self.stderr.write(f"Skipping probability for rarity={row['rarity']} version={row['version']}: {e}")
                    continue
                version, created = Version.objects.get_or_create(name=row['version'])

                obj, created = RarityProbability.objects.update_or_create(
                    rarity=rarity,
                    version=version,
                    defaults={
                ...
                action = "Created" if created else "Updated"
                self.stdout.write(f"{action} RarityProbability: {rarity.name} / {version.name}")